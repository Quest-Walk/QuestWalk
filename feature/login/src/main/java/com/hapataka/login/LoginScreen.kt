...
import com.hapataka.designsystem.theme.HighLightYellow
...
import com.hapataka.feature.login.R
import com.hapataka.login.model.LoginState

@Composable
fun LoginRoute(
    padding: PaddingValues, viewModel: LoginViewModel = hiltViewModel()
) {
    val loginState by viewModel.loginState.collectAsState()

    LoginScreen(
        padding = padding, loginState = loginState, loginWithEmail = viewModel::loginWithEmail
    )
}
        ...
        Column(
            ...
            if (loginState is LoginState.Loading) {
                ...
                CircularProgressIndicator(
                    modifier = Modifier.size(52.dp).padding(top = 40.dp), color = HighLightYellow
                )
            } else {
                Column(
                    horizontalAlignment = Alignment.CenterHorizontally,
