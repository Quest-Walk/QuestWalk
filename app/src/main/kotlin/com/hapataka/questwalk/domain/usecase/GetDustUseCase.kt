...
class GetDustUseCase @Inject constructor(
    ...
    private fun convertToBesselLocation(location: LocationEntity): Location {
        ...
        val currentLocation =
            ProjCoordinate(location.location.second.toDouble(), location.location.first.toDouble())
        val transformLocation = ProjCoordinate()

        val coordinateTransform =
            CoordinateTransformFactory().createTransform(wgs84System, besselSystem)
    ...
    private fun requestStationQueryMap(besselLocation: Location): Map<String, String> {
        return mapOf(
            "serviceKey" to com.hapataka.questwalk.BuildConfig.weather_key,
            "returnType" to "json",
