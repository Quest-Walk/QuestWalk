...
class GoogleMapRepositoryImpl : MapRepository, OnMapReadyCallback {
    ...
    override fun drawPath(result: History.QuestResult) {
        ...
        val tAnimator =
            ValueAnimator.ofInt(0, if (locationList.size < 2) 0 else locationList.size - 2).apply {
                ...
                addUpdateListener { tAnimator ->
                    ...
                    val nextFrameValue = if (locationList.size < 2) {
                        LatLng(
                            locationList[frame].latitude + 0.000001,
                            locationList[frame].longitude + 0.000001
                        )
                    } else {
